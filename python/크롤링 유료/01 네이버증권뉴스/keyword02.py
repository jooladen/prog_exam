...
#for i in range(0, len(keyword_lists)):    
    ...
    if(rescode==200):
        response_body = response.read()
        print("count0 >>> ", len(response_body))
        ...
        print("postdate >>> ", result['items'][i]['postdate'])
        print("title >>> ", result['items'][i]['title'])
    else:
        print("Error Code:" + rescode)
...
for t in range(0, len(keyword_lists)):
    block1 = keyword_lists[t]
    block2 = str(PC_mount[t])
    block3 = str(MO_mount[t])
    block4 = str(search_result_mount[t])
    ...
    print(block1 + " " + block2 + " " + block3 + " " + block4 + " " + block5)    
